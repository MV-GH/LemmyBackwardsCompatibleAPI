...
import it.vercruysse.lemmyapi.v0.x19.x11.datatypes.GetPersonDetails
...
internal class LemmyApiUniWrapper(client: HttpClient, apiBaseUrl: String, actualVersion: Version, baseUrl: String, auth: String?) :
    ...
    override suspend fun markPostAsRead(form: it.vercruysse.lemmyapi.datatypes.MarkPostAsRead): Result<it.vercruysse.lemmyapi.datatypes.PostResponse> =
        api.markPostAsRead(
            it.vercruysse.lemmyapi.v0.x19.x11.datatypes.MarkPostAsRead(
                post_ids = listOf(form.post_id),
                read = form.read,
            ),
        ).fold(
            onSuccess = { _ ->
                api.getPost(it.vercruysse.lemmyapi.v0.x19.x11.datatypes.GetPost(id = form.post_id))
    ...
    override suspend fun hidePost(form: it.vercruysse.lemmyapi.datatypes.HidePost): Result<it.vercruysse.lemmyapi.datatypes.PostResponse> =
        api.hidePost(it.vercruysse.lemmyapi.v0.x19.x11.datatypes.HidePost(post_ids = listOf(form.post_id), hide = form.hide)).fold(
            onSuccess = { _ ->
                api.getPost(it.vercruysse.lemmyapi.v0.x19.x11.datatypes.GetPost(id = form.post_id))
                    .map(transformer::toUni)
                    .map { post -> it.vercruysse.lemmyapi.datatypes.PostResponse(post_view = post.post_view) }
    ...
    override suspend fun uploadCommunityBanner(
        ...
        uploadAndApplyImage(image) { imageUrl ->
            api.editCommunity(
                it.vercruysse.lemmyapi.v0.x19.x11.datatypes.EditCommunity(
    ...
    override suspend fun uploadSiteBanner(image: ByteArray): Result<it.vercruysse.lemmyapi.datatypes.UploadImageResponse> =
        uploadAndApplyImage(image) { imageUrl ->
            api.editSite(
                it.vercruysse.lemmyapi.v0.x19.x11.datatypes.EditSite(
                    banner = imageUrl,
                ),
            ).map {}
        }
    ...
    override suspend fun uploadUserAvatar(image: ByteArray): Result<it.vercruysse.lemmyapi.datatypes.UploadImageResponse> =
        uploadAndApplyImage(image) { imageUrl ->
            api.saveUserSettings(
                it.vercruysse.lemmyapi.v0.x19.x11.datatypes.SaveUserSettings(
                    avatar = imageUrl,
                ),
            )
        }
    ...
    override suspend fun deleteUserAvatar(): Result<Unit> =
        api.saveUserSettings(it.vercruysse.lemmyapi.v0.x19.x11.datatypes.SaveUserSettings(avatar = ""))

    override suspend fun deleteUserBanner(): Result<Unit> =
        api.saveUserSettings(it.vercruysse.lemmyapi.v0.x19.x11.datatypes.SaveUserSettings(banner = ""))
