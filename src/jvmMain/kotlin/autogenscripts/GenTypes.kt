...
suspend fun downloadTypes(version: String, vShort: String) {
    ...
    withContext(Dispatchers.IO) {
        ...
        for (f in dest.listFiles()!!) {
            if (f.name.endsWith(".ts") && f.isFile() && f.name != "others.ts") {
                ...
                val lines = typeFile.readText()
                    ...
                    .map { line ->
                        ...
                        k = k.replace(Regex("""\b(var)\b"""), "val")
                            ...
                            .replace(Regex("""\b(Number)\b"""), "Int")
                            // Add the CustomDataTypes, that aren't autogenerated
                            .replace(Regex("listing_type: String")) { _ ->
                                imports = "import ${rootPackage}ListingType\n" + imports
                                "listing_type: ListingType"
                            }
                        k = when (fileName) {
                            "SearchResponse", "Search" -> {
                                k.replace(Regex("type_: String")) { _ ->
                                    imports = "import ${rootPackage}SearchType\n" + imports
                                    "type_: SearchType"
                                }
                            }
                            "GetModlog" -> {
                                k.replace(Regex("type_: String")) { _ ->
                                    imports = "import ${rootPackage}ModlogActionType\n" + imports
                        ...
                        if (fileName == "GetComments" || fileName == "GetPersonMentions" || fileName == "GetReplies") {
                            k.replace(Regex("sort: String")) { _ ->
                                imports = "import ${rootPackage}CommentSortType\n" + imports
                                "sort: CommentSortType"
                            }
                        } else {
                            k.replace(Regex("sort: String")) { _ ->
                                imports = "import ${rootPackage}SortType\n" + imports
                                "sort: SortType"
                            ...
                            .replace(Regex("registration_mode: String")) { _ ->
                                imports = "import ${rootPackage}RegistrationMode\n" + imports
                                "registration_mode: RegistrationMode"
                            }
                            .replace(Regex("subscribed: String")) { _ ->
                                imports = "import ${rootPackage}SubscribedType\n" + imports
                                "subscribed: SubscribedType"
                            }
                            .replace(Regex("feature_type: String")) { _ ->
                                imports = "import ${rootPackage}PostFeatureType\n" + imports
